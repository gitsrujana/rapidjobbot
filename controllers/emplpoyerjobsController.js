...
const employerJobPostSchema = Joi.object({
  ...
  experienceLevel: Joi.string()
    .valid("Any", "Freshers Only", "Experienced only")
    .required(),
...
export const registerEmployerJobPost = async (req, res) => {
  ...
  try {
    const existingJobPost = await employerjobs.findOne({
      where: { jobTitle, jobLocation },
    });
    if (existingJobPost) {
      return res
        .status(400)
        .json({
          message: "A job post with this title and location already exists.",
        });
    }
