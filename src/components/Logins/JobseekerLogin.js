...
const JobseekerLogin = () => {
  ...
  const [anchorEl, setAnchorEl] = useState(null); // Popover anchor for social media login
  const [socialPlatform, setSocialPlatform] = useState(""); // Tracks which platform (google, facebook, github)
  ...
  const handleSocialMediaLogin = (event, platform) => {
    setSocialPlatform(platform); // Set the social platform
  ...
  const handlePopupLogin = () => {
    // Redirect user to the respective social media login page
    ...
    <Box sx={{ display: "flex", height: "100vh", justifyContent: "center", alignItems: "center" }}>
     ...
     <Typography  sx={{ marginLeft:"-75%", fontWeight: 'bold' }}>Password</Typography>
          ...
          <Typography variant="body2">
            Not a member yet?{" "}
            <Link href="/homepage" variant="body2" color="primary">
              REGISTER
            </Link>{" "}
      ...
      {/* Popover for Social Media Login Confirmation */}
      <Popover
        id={popoverId}
        open={openPopover}
        anchorEl={anchorEl}
